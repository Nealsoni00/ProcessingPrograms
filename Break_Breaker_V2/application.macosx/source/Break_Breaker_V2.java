...
public class Break_Breaker_V2 extends PApplet {
...
class Brick {
  ...
  Brick(int ix, int iy, int iw, int ih, int ic, int il) {
    ...
    a = PApplet.parseInt(random(-15, 15)); //make each crack in the brickunique
    b = PApplet.parseInt(random(-5, 5)); //same
    d = PApplet.parseInt(random(0, 15));//same
    e = PApplet.parseInt(random(-5, 5));
  }
  public void draw() {
    stroke(0);
    strokeWeight(1);
...
class Ball { //ball class
  ...
  public void initialize(int ix, int iy) { //reset all the variables
    x = ix;
    y = iy;
    vy = 5;
    int[] sign = {
  ...
  public boolean collideTopOrBottom(Paddle p) { // top or bottom of the paddle collision checker
    return((abs(p.y - p.h/2 - y) <= r || abs(p.y + p.h/2 - y) <= r) && x >= p.x - p.w/2 && x <= p.x + p.w/2);
  }
  public boolean collideLeftOrRight(Paddle p) { // left or right of the paddle collision checker
    return((abs(p.x - p.w/2 - x) <= r || abs(p.x + p.w/2 - x) <= r) && y >= p.y - p.h/2 && y <= p.y + p.h/2);
  }
  public boolean collideCorners(Paddle p) { // corners of the paddle collision checker
  ...
  public boolean collideTopOrBottomB(Brick p) { //top or bottom of the brick "p" collision checker
    return((abs(p.y - p.h/2 - y) <= r || abs(p.y + p.h/2 - y) <= r) && x >= p.x - p.w/2 && x <= p.x + p.w/2);
  }
  public boolean collideLeftB(Brick p) { // left (just so the cracks are right
    return(abs(p.x - p.w/2 - x) <= r && y >= p.y - p.h/2 && y <= p.y + p.h/2);
  }
  public boolean collideRightB(Brick p) { // right (same reason as above)
    return(abs(p.x + p.w/2 - x) <= r && y >= p.y - p.h/2 && y <= p.y + p.h/2);
  }
  public boolean collideCornersB(Brick p) { // corners of the brick
    return(distance(x, y, p.x - p.w/2, p.y - p.h/2) <= r || distance(x, y, p.x + p.w/2, p.y - p.h/2) <= r || distance(x, y, p.x - p.w/2, p.y + p.h/2) <= r || distance(x, y, p.x + p.w/2, p.y + p.h/2) <= r);
  }
...
public float distance(float x1, float y1, float x2, float y2) {  //distance formula method
  return(sqrt(sq(x2 - x1) + sq(y2 - y1)));
}
public float collide(float c1, float c2, float c3, float c4) { // check if they colide. This is used just for the paddle as there was priviously some errors with glitching. This method fixes that
...
public void reset() { //to reset bricks when you die or new level
  ...
  for (int i=0; i<10; i++) {//y
    for (int j=0; j<10; j++) {//x
      ...
      brick[i*10+j] = new Brick(PApplet.parseInt(bWidth*j+width/20), PApplet.parseInt(bHeight*i+30), PApplet.parseInt(bWidth-2), PApplet.parseInt(bHeight-2), c, level); //create new brick at a scalable location
    }
...
public void keyPressed() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Break_Breaker_V2" };
    if (passedArgs != null) {
