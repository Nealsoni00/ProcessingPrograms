...
public class Building extends BaseEntity {
	...
	public boolean isOver(double px, double py) {
		...
		px -= pos.x;
		py -= pos.y;
		if(Geometry2D.isInsideRectangle_xyxy(minX, minY, maxX, maxY, px, py)){
			for(int i = 0; i < triangle.length && !result; i += 3)
				result = Geometry2D.isInsideTriangle(contour[triangle[i]], contour[triangle[i+1]], contour[triangle[i+2]], px, py);
		}
		return result;
	}
